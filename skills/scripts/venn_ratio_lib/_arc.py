...
class Arc:
    ...
    def length_degrees(self) -> float:
        ...
        if d_angle > 360:
            ...
            return abs(
                d_angle
            )  # Yes, abs() is needed, otherwise we get the weird "-0.0" output in the doctests

    def length_radians(self) -> float:
    ...
    def approximately_equal(self, arc: "Arc", tolerance=NUMERIC_TOLERANCE) -> bool:
        """
        ...
        return bool(
            ...
            and abs(self.from_angle - arc.from_angle) < tolerance
            and abs(self.to_angle - arc.to_angle) < tolerance
        )

    def point_as_angle(self, pt: Point2DInternal) -> float:
        """
    ...
    def contains_angle_degrees(self, angle: float) -> float:
        """
    ...
    def intersect_circle(
        self, center: Point2DInternal, radius: float
    ) -> Sequence[np.ndarray]:
        """
        ...
        isections = [
            [self.sign * (self.point_as_angle(pt) - self.from_angle) % 360.0, pt]
        ...
        if _len < 360.0 - NUMERIC_TOLERANCE:
            for isec in isections:
                if isec[0] > 360.0 - NUMERIC_TOLERANCE:
                    isec[0] = 0.0

        isections = [
            (a, pt[0], pt[1])
            for (a, pt) in isections
            if a < _len + NUMERIC_TOLERANCE or a > 360 - NUMERIC_TOLERANCE
        ]
        isections.sort()
    ...
    def intersect_arc(self, arc: "Arc") -> Sequence[np.ndarray]:
        ...
        isections = [
            pt
            for pt in intersections
            if arc.contains_angle_degrees(arc.point_as_angle(pt))
        ]
        return isections

    def subarc(
        self, from_angle: Optional[float] = None, to_angle: Optional[float] = None
    ) -> "Arc":
        """
        ...
        if abs(d_new_from - d_new_to) < NUMERIC_TOLERANCE:
            d_new_from = d_new_to
        if d_new_to < d_new_from:
            raise ValueError("Subarc to-angle must be smaller than from-angle.")
        if d_new_to > cur_length + NUMERIC_TOLERANCE:
            raise ValueError("Subarc to-angle must lie within the current arc.")
        return Arc(
            self.center,
            self.radius,
            self.from_angle + self.sign * d_new_from,
            self.from_angle + self.sign * d_new_to,
            self.direction,
        )

    def subarc_between_points(
        self,
        p_from: Optional[Point2DInternal] = None,
        p_to: Optional[Point2DInternal] = None,
    ) -> "Arc":
        """
    ...
    def reversed(self) -> "Arc":
        ...
        """
        return Arc(
            self.center, self.radius, self.to_angle, self.from_angle, not self.direction
        )

    def direction_vector(self, angle: float) -> np.ndarray:
        """
    ...
    def fix_360_to_0(self) -> None:
        ...
        >>> a = Arc((0, 0), 1, 0, -NUMERIC_TOLERANCE/2, True)
        >>> a
        ...
        """
        if abs(self.from_angle - self.to_angle) < NUMERIC_TOLERANCE:
            self.from_angle = self.to_angle

    def lies_on_circle(self, center: Point2DInternal, radius: float) -> bool:
        """Tests whether the arc circle's center and radius match the given ones within <NUMERIC_TOLERANCE> tolerance.
        ...
        """
        return (
            np.all(abs(np.asarray(center) - self.center) < NUMERIC_TOLERANCE)
            and abs(radius - self.radius) < NUMERIC_TOLERANCE
        )

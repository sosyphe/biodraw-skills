...
class UpSet:
    ...
    DPI = 300  # standard matplotlib value
    ...
    def plot_matrix(self, ax):
        ...
        styles = [
            [
                ...
                else {"facecolor": "#e8e8e8", "linewidth": 0}
    ...
    def _label_sizes(self, ax, rects, where):
        ...
        if where == "right":
            xlim = ax.get_xlim()
            margin = 0.01 * float(abs(xlim[1] - xlim[0]))
            for rect in rects:
                width = float(rect.get_width()) + float(rect.get_x())
                ax.text(
                    float(width + margin),
                    float(float(rect.get_y()) + float(rect.get_height()) * 0.5),
            ...
            ylim = ax.get_ylim()
            margin = 0.01 * float(abs(ylim[1] - ylim[0]))
            for rect in rects:
                height = float(rect.get_height()) + float(rect.get_y())
                ax.text(
                    float(float(rect.get_x()) + float(rect.get_width()) * 0.5),
                    float(height + margin),

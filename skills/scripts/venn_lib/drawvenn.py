...
def get_labels(data, fill=["number"]):
    ...
    if fill == "logic":
        ...
        labels = {k: str(len(set_collections[k])) for k in set_collections}
    elif fill == "percent":
        labels = {k: f"{100.0 * len(set_collections[k]) / data_size:.1f}%" for k in set_collections}
    elif fill == "number+percent":
        labels = {k: f"{len(set_collections[k])}\n({100.0 * len(set_collections[k]) / data_size:.1f}%)" for k in set_collections}

    return labels

def venn2(fig,ax,labels, names=['A', 'B'], **options):
    """
    ...
    """
    colors = options.get('colors', [defaultcolors[i] for i in range(2)])
...
def venn3(fig,ax,labels, names=['A', 'B', 'C'], **options):
    """
    ...
    return
      pyplot Figure and AxesSubplot object
    """
    colors = options.get('colors', [defaultcolors[i] for i in range(3)])
...
def venn4(fig,ax,labels, names=['A', 'B', 'C', 'D'], **options):
    """
    ...
    return
      pyplot Figure and AxesSubplot object
    """
    colors = options.get('colors', [defaultcolors[i] for i in range(4)])
...
def venn5(fig,ax,labels, names=['A', 'B', 'C', 'D', 'E'], **options):
    """
    ...
    return
      pyplot Figure and AxesSubplot object
    """
    colors = options.get('colors', [defaultcolors[i] for i in range(5)])
...
def venn6(fig,ax, labels, names=['A', 'B', 'C', 'D', 'E', 'F'], **options):
    """
    ...
    return
      pyplot Figure and AxesSubplot object
    """
    colors = options.get('colors', [defaultcolors[i] for i in range(6)])
...
def venn(fig, ax, labels, names=['A', 'B', 'C', 'D', 'E', 'F'], **options):
    if len(labels.keys()) == np.exp2(2)-1:
        fig,ax = venn2(fig,ax,labels, names, **options)
    elif len(labels.keys()) == np.exp2(3)-1:
        fig,ax = venn3(fig,ax,labels, names, **options)
    elif len(labels.keys()) == np.exp2(4)-1:
        fig,ax = venn4(fig,ax,labels, names, **options)
    elif len(labels.keys()) == np.exp2(5)-1:
        fig,ax = venn5(fig,ax,labels, names, **options)
    elif len(labels.keys()) == np.exp2(6)-1:
        fig,ax = venn6(fig,ax,labels, names, **options)
    return fig,ax

def flower_plot(fig,ax,sample, vennlist, corenum, start, a, b, r,ellipse_col,fontsize):
    n = len(sample)
    deg = 360 / n
    ...
    for t in range(n):
        ...
        # Draw ellipse
        ellipse = patches.Ellipse((x, y), width=2*a, height=2*b, angle=angle,
                          color=ellipse_col[t],
                          ec=ellipse_col[t],
                          alpha=.6)
        ...
        # Text for OTU number
        text_x = 5 + 2.5 * np.cos(np.radians(angle))
        text_y = 5 + 2.5 * np.sin(np.radians(angle))
        ax.text(text_x, text_y, vennlist[t],fontsize=fontsize, ha='center', va='center')

        # Label text with rotation
        label_x = 5 + 3.9 * np.cos(np.radians(angle))
        label_y = 5 + 3.9 * np.sin(np.radians(angle))
        if 0 < (angle % 360) < 180:
            rotation = deg * (t - 1) - start
        else:
            rotation = deg * (t - 1) - start
        ax.text(label_x, label_y, sample[t],fontsize=fontsize, rotation=rotation, ha="center", va='center')

    # Draw central circle
    circle = plt.Circle((5, 5), r, color="white", fill=True)
    ax.add_patch(circle)
    ax.text(5, 5, f'Core\n{corenum}',fontsize=fontsize, ha='center', va='center')
    return fig,ax